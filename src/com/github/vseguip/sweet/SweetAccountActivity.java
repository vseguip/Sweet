...
public class SweetAccountActivity extends AccountAuthenticatorActivity {
	...
	/** The Intent extra to signal if we want to create an account or just confirm credentials. **/
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        mAuthtokenType = intent.getStringExtra(PARAM_AUTHTOKEN_TYPE);//Tipo de token a crear
		...
		mButtonValidate.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (username == null || username.length() <= 0) {
					Toast.makeText(SweetAccountActivity.this, "Please enter a valid username", Toast.LENGTH_LONG);
					return;
				}
				if (passwd == null || passwd.length() <= 0) {
					Toast.makeText(SweetAccountActivity.this, "Please enter a valid passwd", Toast.LENGTH_LONG);
					return;
				}
				if (server == null || server.length() <= 0) {
					Toast.makeText(SweetAccountActivity.this, "Please enter a valid URL to use as a server resource",
							Toast.LENGTH_LONG);
					return;
				}
				try {
					...
					sugar.validate(username, passwd, SweetAccountActivity.this, handler);
				} catch (URISyntaxException ex) {
					Toast.makeText(SweetAccountActivity.this, "Please enter a valid URI to use as a server resource",
							Toast.LENGTH_LONG);
				}
			}
		});
		...
		mButtonCancel.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				SweetAccountActivity.this.finish();
			}
		});
		mButtonCreate.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				SweetAccountActivity.this.createAccount();
			}
		});
	}
	...
	protected void createAccount() {
		...
		if (mCreateAccount) {
			...
			serverData.putString(AccountManager.KEY_USERDATA, server);
		...
		if (mAuthtokenType != null && mAuthtokenType.equals(accountType)) {			
	...
	public void onValidationResult(boolean result, String message) {
		if (result == true) {
			...
			// message carries the ID token the SugarCRM sent us back. Save it for account creation
			mIdSession = message;
			mViewF.showNext();
		} else {
